...
class GPX: NSObject, XMLParserDelegate
{
    ...
    private init(url: NSURL, completionHandler: @escaping GPXCompletionHandler) {
        self.url = url
    ...
    private func parse() {
        DispatchQueue.global(qos:.userInitiated).async {
            if let data = NSData(contentsOf: self.url as URL) {
...
//        dispatch_async(dispatch_get_global_queue(UInt32(QOS_CLASS_USER_INITIATED.rawValue), 0)) {
